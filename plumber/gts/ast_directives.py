...
class DirectiveCSR(Directive):
    def __init__(self, attributes: Dict[str, DirectiveAttributeValueParts]) -> None:
        # defaults
        if "r" not in attributes:
            attributes["r"] = [("IDENTIFIER", "F")]
        if "c" not in attributes:
            attributes["c"] = [("DIGITS", 768)]
        if "v" not in attributes:
            attributes["v"] = [("DIGITS", 0)]

        # attribute sanity check
        for name, value_parts in attributes.items():
            if name not in ["r", "c", "v"]:
                raise SyntaxError("Invalid attribute name.")
            else:
                if name in ["c", "v"] and not value_parts[0][0] == "DIGITS":
                    raise SyntaxError("Invalid attribute: must start with DIGITS token.")
                if name == "r" and not (isinstance(value_parts[0][1], str) and re.match(r"^[TF]$", value_parts[0][1])):
                    raise SyntaxError("Invalid attribute: first value token does not match expected pattern for this kind of placeholder.")

        self.read = True if attributes["r"][0][1] == "T" else False

        assert isinstance(attributes["c"][0][1], int)
        self.address: int = attributes["c"][0][1]

        assert isinstance(attributes["v"][0][1], int)
        self.value = attributes["v"][0][1]

    def codegen(self, generator: CodeGenerator) -> None:
        generator.csr(self.read, self.address, self.value)
