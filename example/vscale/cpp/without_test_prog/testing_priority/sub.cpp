...
int main(int argc, char** argv)
{
  ...
  std::cout << "Changes: " << target_changes << std::endl;
...
void clk_forward(const std::unique_ptr<vscale_top> &model, uint16_t clks) {
  for (auto i {0}; i < clks; i++) {
    ...
    if (target != model->rootp->vscale_core__DOT__pipeline__DOT__csr__DOT__mepc) {
      target = model->rootp->vscale_core__DOT__pipeline__DOT__csr__DOT__mepc;
      target_changes++;
    }
